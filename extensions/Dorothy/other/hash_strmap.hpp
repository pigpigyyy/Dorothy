...
/*
			...
			return h;
*/
			size_t h = 2134173 + (k.n << 5) - k.n;
			ptrdiff_t c = k.n - (SP_ALIGN-1);
			ptrdiff_t i = 0;
			for (; i < c; i += SP_ALIGN)
				h = (h << 5) - h + *reinterpret_cast<const align_type*>(k.p + i);
			for (; i < k.n; ++i)
				h = (h << 5) - h + k.p[i];
			return h;
		}
...
template< class Value = ValueOut // ValueOut means empty value, just like a set
		...
		, class ValuePlace = typename if_c<
					!std::is_empty<Value>::value && // ValueInline should be non-empty
...
class hash_strmap
{
	...
	void move_cons_on_raw(Value* dest, Value* src, FastCopy) {
		*reinterpret_cast<ValueRaw*>(dest) = *reinterpret_cast<const ValueRaw*>(src);
	}
#ifdef HSM_HAS_MOVE
	void move_cons_on_raw(Value* dest, Value* src, SafeCopy) {
		new(dest) Value(std::move(*src));
	}
#else
	void move_cons_on_raw(Value* dest, Value* src, SafeCopy) {
		move_cons_on_raw_aux(dest, *src, std::has_nothrow_constructor<Value>());
	}
	typedef true_  has_nothrow_cons_yes;
	typedef false_ has_nothrow_cons_no;
	void move_cons_on_raw_aux(Value* dst_raw, Value& src, has_nothrow_cons_yes) {
		// assume nothrow cons is fast enough, such as stl containers
		new(dst_raw)Value();
		std::swap(*(Value*)dst_raw, src);
		src.~Value();
	}
	void move_cons_on_raw_aux(Value* dst_raw, Value& src, has_nothrow_cons_no) {
...
#endif // HSM_HAS_MOVE
    void move_cons_on_raw(Value* dest, Value* src) {
        move_cons_on_raw(dest, src, CopyStrategy());
    }
	void move_value_on_raw(Node*, Node*, Value* vdest, Value* vsrc, ValueOut) {
		move_cons_on_raw(vdest, vsrc, CopyStrategy());
	}
	void move_value_on_raw(Node* dest, Node* src, Value*, Value*, ValueInline) {
		move_cons_on_raw(&dest->value, &src->value, CopyStrategy());
	}
			...
			if (nDeleted) {
				for (size_t i = 0, n = nNodes; i < n; ++i) {
					...
					if (delmark != oldpn[i].link)
                        move_cons_on_raw(&pn[i].value, &oldpn[i].value);
                }
			} else {
				for (size_t i = 0, n = nNodes; i < n; ++i) {
                    pn[i].offset = oldpn[i].offset;
	...
	void destroy() {
        if (fastlist)
            free(fastlist);
		if (pNodes) {
			if (!std::has_trivial_destructor<Value>::value) {
...
public:
	...
	static const bool is_value_out = std::is_same<ValuePlace, ValueOut>::value;
	static const bool is_value_empty = std::is_empty<Value>::value;
	typedef bool_< // CPU cache line is often 64
		(is_value_out && !is_value_empty) || (sizeof(Node) > 64)
	> sort_by_index;
	typedef false_ sort_by_index_no;
	typedef true_  sort_by_index_yes;
	...
	hash_strmap(const hash_strmap& y)
	  : hash(y.hash), equal(y.equal) {
		...
		if (0 == y.nDeleted || freelist_disabled != fastleng) {
			...
			typedef std::is_same<CopyStrategy, FastCopy> is_fast_copy;
...
private:
	...
	void erase_i_impl(const LinkTp idx, const size_t h) {
...
public:
	void erase_i(const size_t idx) {
		...
		assert(intptr_t(pHash) == hash_cache_disabled || NULL != pHash);
		size_t h = intptr_t(pHash) == hash_cache_disabled ? hash(key(idx)) : pHash[idx];
	...
	void rearrange_nodes_by_int(LinkTp* index) {
		...
		for (size_t i = 0, n = nNodes; i < n; ++i) { // Rearrange
			if (index[i] != i) {
                ...
                if (intptr_t(ph) != hash_cache_disabled)
                    tmphash = ph[next];
				move_cons_on_raw((Value*)tmpvalue, &nth_value(next));
				do {
					assert(next < n);
					curr = next;
					next = index[next];
				...
				move_cons_on_raw(&nth_value(i), (Value*)tmpvalue);
