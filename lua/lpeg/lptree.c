...
static int getsize (lua_State *L, int idx) {
  return (int)((lua_objlen(L, idx) - sizeof(Pattern)) / sizeof(TTree) + 1);
}
...
static void initialrulename (lua_State *L, TTree *grammar, int frule) {
  if (sib1(grammar)->key == 0) {  /* initial rule is not referenced? */
    int n = (int)lua_objlen(L, -1) + 1;  /* index for name */
...
extern void luaopen_lpeg (lua_State *L) {
  luaL_newmetatable(L, PATTERN_T);
  lua_pushnumber(L, MAXBACK);  /* initialize maximum backtracking */
  lua_setfield(L, LUA_REGISTRYINDEX, MAXSTACKIDX);
  luaL_register(L, NULL, metareg);
  lua_newtable(L); // pattern_t lpeg_t
  luaL_register(L, NULL, pattreg);
