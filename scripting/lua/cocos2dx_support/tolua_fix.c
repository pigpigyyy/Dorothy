...
TOLUA_API int toluafix_pushusertype_ccobject(lua_State* L, int refid, void* ptr, const char* type)
{
	...
	if (lua_isnil(L, -1))// ud == nil
	{
		...
		*(void**)lua_newuserdata(L, sizeof(void *)) = ptr;// mt ubox newud
		...
		lua_setfenv(L, -2);
#endif
		tolua_register_ccobject_gc(ptr);
	}
	else
	{
...
TOLUA_API int toluafix_remove_ccobject_by_refid(lua_State* L, int refid)
{
	...
	if (ud == NULL)
	{
		printf("[LUA ERROR] remove CCObject with NULL userdata, refid: %d\n", refid);
		lua_pop(L, 2);
		return -1;
	}
	// clean userdata
	*ud = NULL;
	...
	lua_pushnil(L);// ubox nil
	lua_rawseti(L, -2, refid);// ubox[refid] = nil, ubox
	lua_pop(L, 1);// stack clean
