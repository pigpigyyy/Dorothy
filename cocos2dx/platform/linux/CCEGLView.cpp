...
void CCEGLView::setFrameSize(const CCSize& size)
{
	...
	//check
	CCAssert(size.width!=0&&size.height!=0, "invalid window's size equal 0");
	...
	switch(iDepth)
	{
		...
		{
			/* Updates video mode */
			eResult = (glfwOpenWindow(size.width, size.height, 5, 6, 5, 0, 16, 8, (int)u32GLFWFlags) != false) ? true : false;
		...
		{
			/* Updates video mode */
			eResult = (glfwOpenWindow(size.width, size.height, 8, 8, 8, 0, 16, 8, (int)u32GLFWFlags) != false) ? true : false;
		...
		{
			/* Updates video mode */
			eResult = (glfwOpenWindow(size.width, size.height, 8, 8, 8, 8, 16, 8, (int)u32GLFWFlags) != GL_FALSE) ? true :false;
			break;
		}
	}
	...
	if(eResult)
	{
	  ...
	  //		glfwGetWindowSize(&width, &height);
		...
		CCEGLViewProtocol::setFrameSize(size);		
